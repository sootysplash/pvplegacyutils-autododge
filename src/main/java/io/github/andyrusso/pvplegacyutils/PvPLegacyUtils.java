...
public class PvPLegacyUtils implements ClientModInitializer {
	...
	private static void autoDodge(String text, PvPLegacyUtilsConfig config){
		MinecraftClient client = MinecraftClient.getInstance();
		if (!(client.player == null || client.world == null)) {
			if(client.player.getScoreboardTeam() != null) {
				//System.out.println("we're on the team " + client.player.getScoreboardTeam().getName() + "!");
			}
				...
				if (isStartOfGame) {
					...
					if (!(client.world.getPlayers().size() >= 3)) {
						//System.out.println("just two players!");
						if (config.autoDodge) {
							dodge(config, false);
						}
					}
				}
				if (client.world.getPlayers().size() >= 3 && client.player.getScoreboardTeam() != null) {
					//System.out.println("more than two players!");
					if (config.autoDodgeTeams) {
						dodge(config, true);
					}
				}
		}
	}
	private static void dodge(PvPLegacyUtilsConfig config, boolean inTeams){
		String PlayerToDodge = config.autoDodgePlayer;
		int Dodge = 0;
		String oppteam = null;
		String ourteam = null;
		AbstractClientPlayerEntity playerthing;
		MinecraftClient client = MinecraftClient.getInstance();
		if (!(client.player == null || client.world == null)) {
				try {
					while (Dodge < 20) {
						playerthing = client.world.getPlayers().get(Dodge);
						String oppPlayer = String.valueOf(client.world.getPlayers().get(Dodge).getEntityName());
						if(inTeams){
							oppteam = playerthing.getScoreboardTeam().getName();
							ourteam = client.player.getScoreboardTeam().getName();
							if (Objects.equals(oppteam, ourteam)) {
								//System.out.println(oppPlayer + " is on our team (" + oppteam + ")");
							} else {
								//System.out.println(oppPlayer + " is not on our team (" + oppteam +")");
							}
						}
						//System.out.println("player to dodge(" + PlayerToDodge + ") contains: " + oppPlayer + " is: " + PlayerToDodge.contains(oppPlayer));
						if (PlayerToDodge.contains(oppPlayer)) {
							//System.out.println(oppPlayer + " is an opp");
							if(!inTeams){
								client.getNetworkHandler().sendCommand("leave");
								//System.out.println("sent leave command, another opp evaded!");
							}
							if(inTeams && !Objects.equals(oppteam, ourteam)){
								client.getNetworkHandler().sendCommand("leave");
								//System.out.println("sent leave command, another opp evaded!");
							}
						} else {
							//System.out.println(oppPlayer + " is not an opp");
						}
						Dodge++;
					}
				} catch (Exception e) {
					//System.out.println("out of bounds wah wah");
				}
			}
		}
	private static void dodgeplayer(
			CommandDispatcher<FabricClientCommandSource> dispatcher,
			CommandRegistryAccess _commandRegistryAccess
	) {
		dispatcher.register(ClientCommandManager.literal("dodgeplayer")
					.executes(context -> listplayer(context.getSource())));

		dispatcher.register(ClientCommandManager.literal("dodgeplayer")
					.then(ClientCommandManager.argument("username", StringArgumentType.string())
							.executes(context -> addplayer(context.getSource(), context.getArgument("username", String.class)))));

	}
	private static int addplayer(FabricClientCommandSource source, String context){
		PvPLegacyUtilsConfig config = PvPLegacyUtilsConfig.getInstance();
		if(context.equals(MinecraftClient.getInstance().player.getEntityName())){
			source.sendFeedback(Text.of("You can't dodge yourself! (You won't be able to queue in 1v1s!)"));
		}else {
			config.autoDodgePlayer = (config.autoDodgePlayer + " " + context);
			source.sendFeedback(Text.of("Added player: '" + context + "' to the dodge list!"));
		}
			return 0;
	}
	private static int listplayer(FabricClientCommandSource source){
		PvPLegacyUtilsConfig config = PvPLegacyUtilsConfig.getInstance();
		source.sendFeedback(Text.of("List of players to dodge: '"+config.autoDodgePlayer+"'."));
		return 0;
	}
	...
	private static ItemStack onMiddleClick(PlayerEntity player, HitResult result) {
		...
		/*test for fetching names, i had issues earlier -ss
		if(MinecraftClient.getInstance().player != null && MinecraftClient.getInstance().player.getScoreboardTeam() != null) {
			System.out.println("team is : " + MinecraftClient.getInstance().player.getScoreboardTeam().getName());
		}else{
			System.out.println("team is null!");
		}
		if(MinecraftClient.getInstance().player != null){
			System.out.println("get name " +MinecraftClient.getInstance().player.getName());
			System.out.println("get entity name " +MinecraftClient.getInstance().player.getEntityName());
			System.out.println("get display name " +MinecraftClient.getInstance().player.getDisplayName());
			System.out.println("get custom name " +MinecraftClient.getInstance().player.getCustomName());
		}*/
	...
	public void onInitializeClient() {
		...
		ClientCommandRegistrationCallback.EVENT.register(PvPLegacyUtils::dodgeplayer);
